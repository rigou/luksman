...
if [ "$Vol_option" = '-f' ] ; then
	...
	if ! ./luksman create "$Name" "$(get_vol_option)" "$(get_vol_path)" "$Key_option" "$Key_path" -o "$User" ; then
        exit_error "$@"
    fi
fi
print_line
...
if ! ./luksman delete "$Name" "$(get_vol_option)" "$(get_vol_path)" "$Key_option" "$Key_path" ; then
    exit_error "$@"
fi
print_line
